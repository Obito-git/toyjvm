...
use common::instruction::{LookupSwitchData, TableSwitchData, ArrayType};
use common::Value;
...
#[inline]
pub(super) fn handle_checkcast(thread_id: ThreadId, _idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
...
#[inline]
pub(super) fn handle_aconst_null(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
...
pub(super) fn handle_aload0(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = *vm.get_stack_mut(&thread_id)?.cur_java_frame()?.get_local(0)?;
    vm.get_stack_mut(&thread_id)?.push_operand(value)?;
    Ok(())
}
...
pub(super) fn handle_aload1(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = *vm.get_stack_mut(&thread_id)?.cur_java_frame()?.get_local(1)?;
    vm.get_stack_mut(&thread_id)?.push_operand(value)?;
    Ok(())
}
...
pub(super) fn handle_aload2(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = *vm.get_stack_mut(&thread_id)?.cur_java_frame()?.get_local(2)?;
    vm.get_stack_mut(&thread_id)?.push_operand(value)?;
    Ok(())
}
...
pub(super) fn handle_aload3(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = *vm.get_stack_mut(&thread_id)?.cur_java_frame()?.get_local(3)?;
    vm.get_stack_mut(&thread_id)?.push_operand(value)?;
    Ok(())
}

#[inline]
pub(super) fn handle_aload(thread_id: ThreadId, pos: u8, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = *vm.get_stack_mut(&thread_id)?.cur_java_frame()?.get_local(pos)?;
    vm.get_stack_mut(&thread_id)?.push_operand(value)?;
    Ok(())
}

#[inline]
pub(super) fn handle_anewarray(thread_id: ThreadId, idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    let target_array_sym = vm.method_area.get_cp_by_method_id(&cur_frame_method_id)?.get_class_sym(&idx, vm.interner())?;
    ...
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Ref(array_ref))?;
    Ok(())
}

#[inline]
pub(super) fn handle_arraylength(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let array_ref = vm.get_stack_mut(&thread_id)?.pop_obj_val()?;
    let length = vm.heap.get_array_length(array_ref)?;
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer(length))?;
    Ok(())
}
...
#[inline]
pub(super) fn handle_astore(thread_id: ThreadId, pos: u8, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = vm.get_stack_mut(&thread_id)?.pop_nullable_ref()?;
...
#[inline]
pub(super) fn handle_bipush(thread_id: ThreadId, value: i8, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer(value as i32))?;
    Ok(())
}
...
pub(super) fn handle_castore(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    let index = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let array_ref = vm.get_stack_mut(&thread_id)?.pop_obj_val()?;
    vm.heap.write_array_element(array_ref, index, Value::Integer(value))?;
    Ok(())
}
...
pub(super) fn handle_dadd(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Double(v1 + v2))?;
    Ok(())
}

#[inline]
pub(super) fn handle_dconst0(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Double(0.0))?;
    Ok(())
}

#[inline]
pub(super) fn handle_dconst1(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Double(1.0))?;
    Ok(())
}
...
pub(super) fn handle_fcmpl(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    let res = match v1.total_cmp(&v2) { Ordering::Less => -1, Ordering::Equal => 0, Ordering::Greater => 1 };
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer(res))?;
    Ok(())
}
...
pub(super) fn handle_fconst0(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Float(0.0))?;
    Ok(())
}

#[inline]
pub(super) fn handle_fconst1(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Float(1.0))?;
    Ok(())
}
...
#[inline]
pub(super) fn handle_fload(thread_id: ThreadId, n: u8, vm: &mut VirtualMachine) -> Result<(), JvmError> {
...
#[inline]
pub(super) fn handle_fstore(thread_id: ThreadId, n: u8, vm: &mut VirtualMachine) -> Result<(), JvmError> {
...
#[inline]
pub(super) fn handle_getfield(thread_id: ThreadId, idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let target_obj_ref = vm.get_stack_mut(&thread_id)?.pop_obj_val()?;
    let cur_frame_method_id = vm.get_stack_mut(&thread_id)?.cur_java_frame()?.method_id();
    let field_view = vm.method_area.get_cp_by_method_id(&cur_frame_method_id)?.get_field_view(&idx, vm.interner())?;
    let target_class_id = vm.method_area.get_class_id_or_load(field_view.class_sym)?;
    let target_field = vm.method_area.get_instance_class(&target_class_id)?.get_instance_field(&field_view.name_and_type.into())?;
    let value = vm.heap.read_field(target_obj_ref, target_field.offset, vm.method_area.get_field_descriptor(&target_field.descriptor_id).as_allocation_type())?;
    vm.get_stack_mut(&thread_id)?.push_operand(value)?;
    Ok(())
}

#[inline]
pub(super) fn handle_getstatic(thread_id: ThreadId, idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let cur_frame_method_id = vm.get_stack_mut(&thread_id)?.cur_java_frame()?.method_id();
    let target_field_view = vm.method_area.get_cp_by_method_id(&cur_frame_method_id)?.get_field_view(&idx, vm.interner())?;
    let target_class_id = vm.method_area.get_class_id_or_load(target_field_view.class_sym)?;
    ...
    let actual_static_field_class_id = vm.method_area.resolve_static_field_actual_class_id(target_class_id, &field_key)?;
    let value = vm.method_area.get_class(&actual_static_field_class_id).get_static_field_value(&field_key)?;
    vm.get_stack_mut(&thread_id)?.push_operand(value)?;
    Ok(())
}

#[inline]
pub(super) fn handle_goto(thread_id: ThreadId, offset: i16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let new_pc = Interpreter::branch16(pc, offset);
...
pub(super) fn handle_idiv(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value2 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let value1 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    if value2 == 0 { throw_exception!(ArithmeticException, "/ by zero")? }
...
#[inline]
pub(super) fn handle_if_eq(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let value = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if value == 0 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
    *vm.get_stack_mut(&thread_id)?.pc_mut()? = new_pc;
    Ok(())
}

#[inline]
pub(super) fn handle_if_ge(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let value = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if value >= 0 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
    *vm.get_stack_mut(&thread_id)?.pc_mut()? = new_pc;
...
#[inline]
pub(super) fn handle_if_gt(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let value = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if value > 0 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
    *vm.get_stack_mut(&thread_id)?.pc_mut()? = new_pc;
...
pub(super) fn handle_lcmp(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    let res = match v1.cmp(&v2) { Ordering::Less => -1, Ordering::Equal => 0, Ordering::Greater => 1 };
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer(res))?;
    Ok(())
}
...
#[inline]
pub(super) fn handle_lookupswitch(thread_id: ThreadId, switch: &LookupSwitchData, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let key = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let pc = vm.get_stack(&thread_id)?.pc()?;
    let target_offset = match switch.pairs.binary_search_by_key(&key, |p| p.0) { Ok(i) => switch.pairs[i].1, Err(_) => switch.default_offset };
    let new_pc = Interpreter::branch32(pc, target_offset);
    *vm.get_stack_mut(&thread_id)?.pc_mut()? = new_pc;
    Ok(())
}

#[inline]
pub(super) fn handle_ifnull(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let value = vm.get_stack_mut(&thread_id)?.pop_nullable_ref_val()?;
    let new_pc = if value.is_none() { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
    *vm.get_stack_mut(&thread_id)?.pc_mut()? = new_pc;
    Ok(())
}

#[inline]
pub(super) fn handle_if_icmplt(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let v2 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if v1 < v2 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
    *vm.get_stack_mut(&thread_id)?.pc_mut()? = new_pc;
    Ok(())
}

#[inline]
pub(super) fn handle_if_le(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let value = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if value <= 0 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
    *vm.get_stack_mut(&thread_id)?.pc_mut()? = new_pc;
...
#[inline]
pub(super) fn handle_if_lt(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let value = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if value < 0 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
    *vm.get_stack_mut(&thread_id)?.pc_mut()? = new_pc;
...
#[inline]
pub(super) fn handle_if_acmpeq(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
...
#[inline]
pub(super) fn handle_if_acmpne(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let v2 = vm.get_stack_mut(&thread_id)?.pop_nullable_ref_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_nullable_ref_val()?;
    let new_pc = if v1 != v2 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
...
#[inline]
pub(super) fn handle_if_icmpne(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let v2 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if v1 != v2 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
...
#[inline]
pub(super) fn handle_if_icmpge(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let v2 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if v1 >= v2 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
...
#[inline]
pub(super) fn handle_if_icmpgt(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let v2 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if v1 > v2 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
...
#[inline]
pub(super) fn handle_if_icmpeq(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let v2 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if v1 == v2 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
...
#[inline]
pub(super) fn handle_if_icmple(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let v2 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if v1 <= v2 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
...
#[inline]
pub(super) fn handle_ifnonnull(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let obj = vm.get_stack_mut(&thread_id)?.pop_nullable_ref_val()?;
    let new_pc = if obj.is_some() { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
    *vm.get_stack_mut(&thread_id)?.pc_mut()? = new_pc;
    Ok(())
}

#[inline]
pub(super) fn handle_if_ne(thread_id: ThreadId, offset: i16, instruction_byte_size: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let pc = vm.get_stack_mut(&thread_id)?.pc()?;
    let i = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let new_pc = if i != 0 { Interpreter::branch16(pc, offset) } else { pc + instruction_byte_size as usize };
    *vm.get_stack_mut(&thread_id)?.pc_mut()? = new_pc;
    Ok(())
}
...
#[inline]
pub(super) fn handle_iload(thread_id: ThreadId, pos: u8, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = *vm.get_stack_mut(&thread_id)?.cur_java_frame()?.get_local(pos)?;
    vm.get_stack_mut(&thread_id)?.push_operand(value)?;
    Ok(())
}
...
#[inline]
pub(super) fn handle_invokevirtual(thread_id: ThreadId, idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let cur_frame_method_id = vm.get_stack_mut(&thread_id)?.cur_java_frame()?.method_id();
    let target_method_view = vm.method_area.get_cp_by_method_id(&cur_frame_method_id)?.get_method_view(&idx, vm.interner())?;
    let method_key: MethodKey = target_method_view.name_and_type.into();
    let target_method_desc_id = vm.method_area.get_or_new_method_descriptor_id(&method_key.desc).unwrap();
    let arg_count = vm.method_area.get_method_descriptor(&target_method_desc_id).params.len() + 1;
    let object_ref = vm.get_stack(&thread_id)?.peek_at(arg_count - 1)?.as_obj_ref()?;
    let actual_class_id = vm.heap.get_class_id(object_ref)?;
    let target_method_id = vm.method_area.get_class(&actual_class_id).get_vtable_method_id(&method_key)?;
...
#[inline]
pub(super) fn handle_instanceof(thread_id: ThreadId, idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let cur_frame_method_id = vm.get_stack_mut(&thread_id)?.cur_java_frame()?.method_id();
    let class_name_sym = vm.method_area.get_cp_by_method_id(&cur_frame_method_id)?.get_class_sym(&idx, vm.interner())?;
    ...
    if let Some(obj_ref) = obj_ref {
        ...
        vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer(if res { 1 } else { 0 }))?;
    } else {
        vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer(0))?;
    }
    Ok(())
...
pub(super) fn handle_fmul(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let v2 = vm.get_stack_mut(&thread_id)?.pop_float_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_float_val()?;
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Float(v1 * v2))?;
    Ok(())
}
...
pub(super) fn handle_irem(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let v2 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    if v2 == 0 { throw_exception!(ArithmeticException, "/ by zero")? }
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer(v1 % v2))?;
    Ok(())
}
...
pub(super) fn handle_ladd(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let v2 = vm.get_stack_mut(&thread_id)?.pop_long_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_long_val()?;
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Long(v1.wrapping_add(v2)))?;
    Ok(())
}
...
pub(super) fn handle_ldiv(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    let v1 = vm.get_stack_mut(&thread_id)?.pop_long_val()?;
    if v2 == 0 { throw_exception!(ArithmeticException, "/ by zero")? }
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Long(v1.wrapping_div(v2)))?;
    Ok(())
}
...
pub(super) fn handle_lmul(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let v2 = vm.get_stack_mut(&thread_id)?.pop_long_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_long_val()?;
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Long(v1.wrapping_mul(v2)))?;
    Ok(())
}
...
pub(super) fn handle_lrem(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    let v1 = vm.get_stack_mut(&thread_id)?.pop_long_val()?;
    if v2 == 0 { throw_exception!(ArithmeticException, "/ by zero")? }
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Long(v1 % v2))?;
    Ok(())
}
...
pub(super) fn handle_l2i(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer(v as i32))?;
    Ok(())
}
...
pub(super) fn handle_l2f(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let v = vm.get_stack_mut(&thread_id)?.pop_long_val()?;
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Float(v as f32))?;
    Ok(())
}
...
pub(super) fn handle_d2l(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Long(v as i64))?;
    Ok(())
}
...
pub(super) fn handle_f2d(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let v = vm.get_stack_mut(&thread_id)?.pop_float_val()?;
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Double(v as f64))?;
    Ok(())
}
...
pub(super) fn handle_ineg(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer(-v))?;
    Ok(())
}
...
pub(super) fn handle_i2s(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let v = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer((v as i16) as i32))?;
    Ok(())
}
...
pub(super) fn handle_i2c(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let v = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer((v as u16) as i32))?;
    Ok(())
}
...
pub(super) fn handle_i2b(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let v = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer((v as i8) as i32))?;
    Ok(())
}
...
#[inline]
pub(super) fn handle_istore(thread_id: ThreadId, idx: u8, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = vm.get_stack_mut(&thread_id)?.pop_int()?;
...
pub(super) fn handle_imul(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let v2 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let v1 = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer(v1.wrapping_mul(v2)))?;
    Ok(())
}

#[inline]
pub(super) fn handle_iinc(thread_id: ThreadId, index: u8, const_val: i8, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = vm.get_stack_mut(&thread_id)?.get_local_int_val(index)?;
    vm.get_stack_mut(&thread_id)?.set_local(index as usize, Value::Integer(value + (const_val as i32)))?;
    Ok(())
}

#[inline]
pub(super) fn handle_ldc(thread_id: ThreadId, idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    let ldc_operand = {
        ...
        match cp.get_constant(&idx, vm.interner())? {
            ...
            RuntimeConstant::Class(class_entry) => {
                ...
                Value::Ref(vm.method_area.get_mirror_ref_or_create(class_id, &mut vm.heap)?)
            }
...
#[inline]
pub(super) fn handle_new(thread_id: ThreadId, idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let cur_frame_method_id = vm.get_stack_mut(&thread_id)?.cur_java_frame()?.method_id();
    let target_class_name = vm.method_area.get_cp_by_method_id(&cur_frame_method_id)?.get_class_sym(&idx, vm.interner())?;
    let target_class_id = vm.method_area.get_class_id_or_load(target_class_name)?;
    Interpreter::ensure_initialized(thread_id, Some(target_class_id), vm)?;
    let instance_ref = vm.heap.alloc_instance(vm.method_area.get_instance_class(&target_class_id)?.get_instance_size()?, target_class_id)?;
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Ref(instance_ref))?;
    Ok(())
}

#[inline]
pub(super) fn handle_newarray(thread_id: ThreadId, array_type: ArrayType, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let size = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    if size < 0 { throw_exception!(NegativeArraySizeException, size.to_string())? }
    let class_id = vm.method_area.load_array_class(vm.interner().get_or_intern(array_type.descriptor()))?;
...
#[inline]
pub(super) fn handle_putfield(thread_id: ThreadId, idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    let target_class_id = vm.method_area.get_class_id_or_load(field_view.class_sym)?;
    let target_field = vm.method_area.get_instance_class(&target_class_id)?.get_instance_field(&field_view.name_and_type.into())?;
    vm.heap.write_field(target_obj_ref, target_field.offset, value, vm.method_area.get_field_descriptor(&target_field.descriptor_id).as_allocation_type())?;
    Ok(())
}

#[inline]
pub(super) fn handle_putstatic(thread_id: ThreadId, idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = vm.get_stack_mut(&thread_id)?.pop_operand()?;
    let cur_frame_method_id = vm.get_stack_mut(&thread_id)?.cur_java_frame()?.method_id();
    let target_field_view = vm.method_area.get_cp_by_method_id(&cur_frame_method_id)?.get_field_view(&idx, vm.interner())?;
    let target_class_id = vm.method_area.get_class_id_or_load(target_field_view.class_sym)?;
    ...
    let field_key: FieldKey = target_field_view.name_and_type.into();
    let actual_static_field_class_id = vm.method_area.resolve_static_field_actual_class_id(target_class_id, &field_key)?;
    vm.method_area.get_class_like(&actual_static_field_class_id)?.set_static_field_value(&field_key, value)?;
    Ok(())
}

#[inline]
pub(super) fn handle_invokeinterface(thread_id: ThreadId, idx: u16, count: u8, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let cur_frame_method_id = vm.get_stack_mut(&thread_id)?.cur_java_frame()?.method_id();
    let target_method_view = vm.method_area.get_cp_by_method_id(&cur_frame_method_id)?.get_interface_method_view(&idx, vm.interner())?;
    let object_ref = vm.get_stack(&thread_id)?.peek_at(count as usize - 1)?.as_obj_ref()?;
    if target_method_view.class_sym == vm.interner().get_or_intern("jdk/internal/access/JavaLangRefAccess") && target_method_view.name_and_type.name_sym == vm.interner().get_or_intern("startThreads") {
        warn!("TODO: Stub: Ignoring call to jdk/internal/access/JavaLangRefAccess.startThreads");
        for _ in 0..count { let _ = vm.get_stack_mut(&thread_id)?.pop_operand()?; }
    } else {
        let target_class_id = vm.heap.get_class_id(object_ref)?;
        let target_method_id = vm.method_area.get_instance_class(&target_class_id)?.get_interface_method_id(&target_method_view.name_and_type.into())?;
        let args = Interpreter::prepare_method_args(thread_id, target_method_id, vm)?;
        Interpreter::invoke_method_internal(thread_id, target_method_id, args, vm)?;
    }
    Ok(())
...
#[inline]
pub(super) fn handle_invokespecial(thread_id: ThreadId, idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let cur_frame_method_id = vm.get_stack_mut(&thread_id)?.cur_java_frame()?.method_id();
    let target_method_view = vm.method_area.get_cp_by_method_id(&cur_frame_method_id)?.get_method_view(&idx, vm.interner())?;
    let target_class_id = vm.method_area.get_class_id_or_load(target_method_view.class_sym)?;
    let target_method_id = vm.method_area.get_instance_class(&target_class_id)?.get_special_method_id(&target_method_view.name_and_type.into())?;
    let args = Interpreter::prepare_method_args(thread_id, target_method_id, vm)?;
    Interpreter::invoke_method_internal(thread_id, target_method_id, args, vm)?;
    Ok(())
}
...
#[inline]
pub(super) fn handle_invokestatic(thread_id: ThreadId, idx: u16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let cur_frame_method_id = vm.get_stack_mut(&thread_id)?.cur_java_frame()?.method_id();
    let target_method_view = vm.method_area.get_cp_by_method_id(&cur_frame_method_id)?.get_method_view(&idx, vm.interner())?;
    let target_class_id = vm.method_area.get_class_id_or_load(target_method_view.class_sym)?;
    Interpreter::ensure_initialized(thread_id, Some(target_class_id), vm)?;
...
#[inline]
pub(super) fn handle_lload(thread_id: ThreadId, pos: u8, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = *vm.get_stack_mut(&thread_id)?.cur_java_frame()?.get_local(pos)?;
    vm.get_stack_mut(&thread_id)?.push_operand(value)?;
    Ok(())
}
...
pub(super) fn handle_lshl(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    ...
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Long(result))?;
    Ok(())
}
...
#[inline]
pub(super) fn handle_lstore(thread_id: ThreadId, idx: u8, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let value = vm.get_stack_mut(&thread_id)?.pop_long()?;
    vm.get_stack_mut(&thread_id)?.set_local(idx as usize, value)?;
    Ok(())
}
...
#[inline]
pub(super) fn handle_sipush(thread_id: ThreadId, value: i16, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    vm.get_stack_mut(&thread_id)?.push_operand(Value::Integer(value as i32))?;
    Ok(())
}

#[inline]
pub(super) fn handle_tableswitch(thread_id: ThreadId, switch: &TableSwitchData, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let index = vm.get_stack_mut(&thread_id)?.pop_int_val()?;
    let pc = vm.get_stack(&thread_id)?.pc()?;
...
#[inline]
pub(super) fn handle_return(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<Option<Value>, JvmError> {
    let _ = (thread_id, vm);
    Ok(None)
}

#[inline]
pub(super) fn handle_ireturn(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<Option<Value>, JvmError> {
    let ret_value = vm.get_stack_mut(&thread_id)?.pop_int()?;
    Ok(Some(ret_value))
}

#[inline]
pub(super) fn handle_areturn(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<Option<Value>, JvmError> {
    let value = vm.get_stack_mut(&thread_id)?.pop_nullable_ref()?;
    Ok(Some(value))
}

#[inline]
pub(super) fn handle_lreturn(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<Option<Value>, JvmError> {
    let value = vm.get_stack_mut(&thread_id)?.pop_long()?;
    Ok(Some(value))
}

#[inline]
pub(super) fn handle_freturn(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<Option<Value>, JvmError> {
    let value = vm.get_stack_mut(&thread_id)?.pop_float()?;
    Ok(Some(value))
}

#[inline]
pub(super) fn handle_monitorenter(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let _obj = vm.get_stack_mut(&thread_id)?.pop_obj_val()?;
    Ok(())
}

#[inline]
pub(super) fn handle_monitorexit(thread_id: ThreadId, vm: &mut VirtualMachine) -> Result<(), JvmError> {
    let _obj = vm.get_stack_mut(&thread_id)?.pop_obj_val()?;
    Ok(())
}
